...
/*
select AddDt, count(*) from Car.Car group by AddDt order by AddDt desc;

select Year, count(*) from Car.Car group by Year order by Year

select * from Car.Car where Year = '2016'

select Wireless, count(*) from Car.Car group by Wireless;
*/
...
CREATE PROC [Rpt].[up_StateCarCnt]
